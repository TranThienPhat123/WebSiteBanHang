...
// Đảm bảo $errors không bị undefined
$errors = $errors ?? [];
...
<form method="POST" action="/project1/Product/update" enctype="multipart/form-data" onsubmit="return validateForm();">
    ...
    <div class="form-group">
        ...
        <input type="number" id="price" name="price" class="form-control" step="0.01"
               value="<?php echo number_format((float)$product->price, 2, '.', ''); ?>" required>
    </div>
    ...
    <div class="form-group">
        <label for="image">Hình ảnh (bỏ qua nếu không đổi):</label>
        <input type="file" id="image" name="image" class="form-control" accept="image/*">
        <!-- Giữ tên ảnh cũ trong input ẩn -->
        <input type="hidden" name="existing_image" value="<?php echo htmlspecialchars($product->image, ENT_QUOTES, 'UTF-8'); ?>">

        <?php if (!empty($product->image)): ?>
            <div class="mt-2">
                <img src="/project1/uploads/<?php echo htmlspecialchars($product->image, ENT_QUOTES, 'UTF-8'); ?>" alt="Ảnh sản phẩm" style="max-width: 100px;">
            </div>
        <?php endif; ?>
    </div>
